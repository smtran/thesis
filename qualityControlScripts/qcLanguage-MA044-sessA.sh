...
# scanner. 
      ...
      echo -n "FILE COUNT: "
      ls -1 ${sourceParentDir}/COG.C.${participantNumber}.cowat.nii | wc -l
      echo "If that's the expected file count for the series, hit "
      echo -n "Enter to continue. (or CTRL-C to quit)"
      read
      echo "Creating .bxh metafile for COG.C.${participantNumber}.cowat.nii..."
      echo ""
      analyze2bxh ${sourceParentDir}/COG.C.${participantNumber}.cowat.nii ${tempDir}/epi01.bxh
      echo ""
      echo "...done."
      echo ""
      echo "Creating new nifti volume for sequence COG.C.${participantNumber}.cowat.nii"
      echo ""
