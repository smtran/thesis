...
currentFile="/data/birc/Atlanta/tranThesis/03.derivedData/denoiseFIX/${sessionType}/MA${blind}_preprocessMELODIC.ica/design.fsf"

echo " "
echo "###########################"
echo "blind=${blind}"
echo "sessionType=${sessionType}"
smoothingKernal="`grep "set fmri(smooth)" ${currentFile} | awk '{print $3}'`"
echo "smoothness=${smoothingKernal}mm"
